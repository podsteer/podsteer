...
describe('a template container\'s environment', () => {
  it('shows a downward-API value resolved, marked, and a literal unmarked', () => {
    ...
    expect(text).toContain('ENVIRONMENT development (resolved)')
    expect(text).toContain('LITERAL development')
    expect(text).not.toContain('LITERAL development (resolved)')
    // A template's pod has no name yet, so the path stays.
    expect(text).toContain('POD_NAME <metadata.name>')
    expect(container.querySelectorAll('[data-row-suffix]')).toHaveLength(1)
  })
})
