...
// WorkloadUsage is what a controller's pods are using right now, against what
// they reserved and what they will be stopped at.
//
// A CONTROLLER HAS NO USAGE OF ITS OWN. It is a template and a replica count;
// the consumption belongs to whatever pods it currently has, which is why this
// is derived from them rather than read from the controller. That also decides
// what the figures mean: a Deployment scaled to zero is using nothing because
// nothing of it is running, and a CronJob between runs is the same — which is
// a true and useful thing for a chart to show, not a gap in it.
type WorkloadUsage struct {
	// Pods is how many pods the figures were summed over.
	Pods int
	// Measured is how many of those reported a measurement.
	//
	// A COUNT RATHER THAN A FLAG, because it is the caveat: a Deployment of
	// twenty pods where metrics-server answered for eighteen is showing usage
	// that is real but short, and a chart that says "1.2 cores" without
	// saying so is overstating what it knows.
	Measured int
	// Usage is the sum over the pods that reported one.
	Usage Metrics
	// Requests and Limits are summed over the pods that OCCUPY A NODE, which
	// is not every pod: a Succeeded Job pod still exists and still counts
	// above, and has given its reservation back.
	Requests Resources
	Limits   Resources
}

// HasMetrics reports whether anything measured at all.
func (u WorkloadUsage) HasMetrics() bool { return u.Measured > 0 }
...
func (u WorkloadUsage) Partial() bool { return u.Measured > 0 && u.Measured < u.Pods }

// NewWorkloadUsage sums a controller's pods into one reading.
func NewWorkloadUsage(pods []Pod) WorkloadUsage {
	usage := WorkloadUsage{Pods: len(pods)}
