...
func (m *ManagementAPI) ResizeContainer(clusterID, namespace, podName, container, cpuRequest, cpuLimit, memoryRequest, memoryLimit string) (ResizeResult, error) {
	ctx, cancel := m.app.requestContext()
	defer cancel()

	id, err := domain.NewClusterID(clusterID)
	if err != nil {
		return ResizeResult{}, apiError(m.logger, "ResizeContainer", err)
	}
	...
	plan, err := m.management.ResizeContainer(ctx, id, ns, podName, domain.ResizeRequest{
		Container:     container,
	...
	return ResizeResult{
		...
		RestartReason: plan.RestartReason,
	}, nil
}
