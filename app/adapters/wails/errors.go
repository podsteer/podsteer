...
const (
	...
	CodeVendorCLIMissing ErrorCode = "vendor_cli_missing"
	// CodeVendorCLIDeclined means the CLI ran and would not answer.
	CodeVendorCLIDeclined ErrorCode = "vendor_cli_declined"
	// CodeVendorCLITimedOut means it did not finish and was stopped.
	CodeVendorCLITimedOut ErrorCode = "vendor_cli_timed_out"
	// CodeVendorCLIUnreadable means it printed something PodSteer will not
	// decode.
	CodeVendorCLIUnreadable ErrorCode = "vendor_cli_unreadable"
...
func classifyError(err error) (ErrorCode, string) {
	...
	// BEFORE THE TRANSPORT CASES TOO, and for the same reason as the
	// credential plugin below: no cluster was contacted. A cloud CLI is a
	// program on the operator's machine, and reporting its absence or its
	// refusal as a cluster problem sends somebody to check a VPN over a
	// program that is not installed or an account they have not signed in to.
	case errors.Is(err, ports.ErrVendorCLIMissing):
		return CodeVendorCLIMissing, err.Error()

	case errors.Is(err, ports.ErrVendorCLIDeclined):
		// THE CLI'S OWN WORDS, whole. The message is the error's text rather
		// than a sentence written here, because PodSteer knows less about why
		// that CLI declined than the CLI does.
		return CodeVendorCLIDeclined, err.Error()

	case errors.Is(err, ports.ErrVendorCLITimedOut):
		return CodeVendorCLITimedOut, err.Error()

	case errors.Is(err, ports.ErrVendorCLIUnreadable):
		return CodeVendorCLIUnreadable, err.Error()
